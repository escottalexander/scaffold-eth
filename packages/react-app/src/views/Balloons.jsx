...
export default function Balloons({ balloons }) {
  ...
  const dispersedBalloons = balloons.map(balloon => {
    ...
    const wobbleEffectNum = oneThroughFour(
      numFromHash(balloon.attributes.find(a => a.trait_type === "color1").value, 2),
    );
    ...
    return (
      <img
        key={id + "-" + balloon.name}
        src={balloon.image}
        className={"wobble-" + wobbleEffectNum}
        width="100"
        style={{ position: "absolute", left: balloonX + "px", top: balloonY + "px" }}
      />
    );
  });
